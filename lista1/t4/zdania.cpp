...
const string z4 = "judyta dała wczoraj Stefanowi czekoladki";
const string z5 = "babuleńka miała dwa rogate koziołki";
vector<string> zdania{z1, z2, z3, z4, z5};
...
long long mult_apps(
  ...
  long long all_matches = 1;
  for (int i = 1; i < words.size(); i++) {
    string key = words[i-1];
    string value = words[i];
    for (int j = 0; j < gram2[key].size(); j++) {
      if (gram2[key][j].first == value)
        all_matches *= (long long)gram2[key][j].second;
    }
  }
  ...
  for (int i = 2; i < words.size(); i++) {
    ...
    for (int j = 0; j < gram3[key].size(); j++) {
      if (gram3[key][j].first == value)
        all_matches *= (long long)gram3[key][j].second;
    }
  }
  return all_matches;
}
...
long long eval_perm(
  ...
  return mult_apps(words, gram2, gram3) + (long long)sum_apps(words, gram2, gram3);
